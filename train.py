...
class PPO:
    ...
    def save(self, name=""):
        torch.save(self.actor, name + "agent.pkl")
...
if __name__ == "__main__":
    ...
    for i in range(ITERATIONS):
        ...
        if (i + 1) % (ITERATIONS // 100) == 0:
            ...
            if np.mean(rewards) > 1200 and np.std(rewards) < 100:
                ppo.save(name="best1200_")
            if np.mean(rewards) > 1100 and np.std(rewards) < 100:
                ppo.save(name="best1100_")
            if np.mean(rewards) > 1000 and np.std(rewards) < 100:
                ppo.save(name="best1000_")
            if np.mean(rewards) > 900:
                ppo.save(name="best900_")
            # if np.mean(rewards) > 850:
            #     ppo.save(name="best850_")
            # if np.mean(rewards) > 800:
            #     ppo.save(name="best800_")
            ppo.save()
